...
Environment::Environment(int width, int height, Point start, Point goal, map <char, PieceGrid> dictShape, map <char, int> dictNumber, Tree t, int alg, char **paths){
...
bool Environment::Solve_DFS(Tree tr){
    ...
    if(alg == 1){
        ...
        if(CheckGoal()){
            if(FullCheck()){
                tr -> id = n_sol;
                strcpy(temp, GetPath(tr).c_str());
                strcpy(paths[n_sol], temp);
                n_sol++;
            }
...
void Environment::CoveredByItself(char board[10][10]){
    int sum_pieces = 0;
    char grid[10][10];
    ...
    for(int sec = 0; sec < s -> n_sec; sec++){
        ...
        for(int i = 0; i < s -> height - 1; i++){
            for(int j = 0; j < s -> width - 1; j++){
                if(s -> grid[i][j].section == sec && board[i][j] != '-')
                    sum_pieces += dictNumber[board[i][j]];
            }
        }
...
//check if the number of sections with pieces for 3 boards is more than 2 and if the number of 
//os sqrs in the pieces of each board are different
int Environment::PostCheck(char board1[10][10], char board2[10][10], string path){
    int sec = 0;
    int result = 0;
    bool flag;
    int counter = 0;
    vector<char> pieceTypes[3];
    ...
    if(counter == 1)
        ...
        result += (counter - 1) * 300;
    
    //count the number of sqrs in the pieces
    int sqrs1, sqrs2, sqrs3;
    sqrs1 = sqrs2 = sqrs3 = 0;
    
    for(int i = 0; i < s -> height - 1; i++){
        for(int j = 0; j < s -> width - 1; j++){
            if(s -> grid[i][j].piece != '-')
                sqrs1 += dictNumber[s -> grid[i][j].piece];
            
            if(board1[i][j] != '-')
                sqrs2 += dictNumber[board1[i][j]];
            
            if(board2[i][j] != '-')
                sqrs3 += dictNumber[board2[i][j]];
        }
    }
    ...
    //check covered by itself
    char board0[10][10];
    
    for(int i = 0; i < s -> height - 1; i++){
        for(int j = 0; j < s -> width - 1; j++){
            board0[i][j] = s -> grid[i][j].piece;
        }
    }
    
    counter = 0;
    
    CoveredByItself(board0);
    counter -= ((float) reached / (float) f_sol) * 50;
    
    CoveredByItself(board1);
    counter -= ((float) reached / (float) f_sol) * 50;
    
    CoveredByItself(board2);
    counter -= ((float) reached / (float) f_sol) * 50;
    
    result += counter;
    
    //check number of pieces in the same places between boards
    counter = 0;
